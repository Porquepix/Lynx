...
public class LynxLogger {
	...
	if (isInfoEnabled()) {
	    ...
	    info("Runtime Environment vendor: {}",
		    System.getProperty("java.vendor"));
	    info("JVM implementation vendor: {}",
		    System.getProperty("java.vm.vendor"));
	    
	    // Hardware
	    info("Available processors (cores): {}", Runtime.getRuntime()
		    .availableProcessors());
	    info("Free memory (bytes): {}", Runtime.getRuntime().freeMemory());
	    long maxMemory = Runtime.getRuntime().maxMemory();
	    info("Maximum memory (bytes): {}",
		    (maxMemory == Long.MAX_VALUE ? "no limit" : maxMemory));
	    info("Total memory available to JVM (bytes): {}", Runtime.getRuntime()
		    .totalMemory());
